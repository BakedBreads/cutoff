...
- **Lockout.** Optionally, reopening that app for the next N minutes puts the screen
  straight back up.
- **Your words.** The big line and the small line under it are both editable in
  Settings, with a "see it full screen" preview.
...
| Lockout on reopen | yes | no — iOS can't see what you open |
...
You need [Node](https://nodejs.org) and a free
[Expo account](https://expo.dev/signup). Nothing else — EAS builds in the cloud, so
no Android Studio and no Xcode.
...
src/
  ...
  components/              Hard, HardPress, Button, Field, Toggle, Sheet, AppTile
...
- **Heads up about this machine:** there's a stray `node_modules` folder sitting
  directly in `C:\Users\you\`. Node walks up the directory tree when resolving,
